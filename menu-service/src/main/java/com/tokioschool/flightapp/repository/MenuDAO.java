...
public interface MenuDAO extends MongoRepository<Menu, String> {
  ...
  List<Menu> findByVegetarianTrueOrderByTitle();
  ...
  Optional<Menu> findByIdWithMainsOrdered(String menuId);
  ...
  @Query(
      """
        { mains: { $elemMatch: {  name: {  $regex: '?0', $options: 'i' } } } }
        """)
  ...
  // List<Menu> findByBeerStyle(String lightLager);
